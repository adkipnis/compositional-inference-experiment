...
dualTrialsClean = merge(dualTrials, limits, by = "id") %>%
  filter(rt >= lower, rt <= upper) %>% select(-map_1) %>% na.omit()
...
-   <s> A random slope may be useful for the factor `acc`.</s>
...
```{r}
ri_model = lme(log(rt) ~ trialNumTotal + acc + inter_RT + cue_type + double_spell + test_type,
...
#| warning: false
gd = groupedData(
  log(inter_RT) ~ cue_type + double_spell + reducible + test_type + trans_ub + acc + trialNumTotal|id, data = dualTrialsClean)
lm_separate = nlme::lmList(
...
```{r}
ri_model = lme(log(inter_RT) ~ cue_type + double_spell + reducible + trans_ub + acc + trialNumTotal,
               random = ~ 1 | id, data = dualTrialsClean)
...
Can individual time-trends for single spells predict those of corresponding double spells?

```{r}
# | warning: false
# weigh later observations higher than earlier ones
profiles = dualTrialsClean %>% filter(map_type == "primitive") %>%
  group_by(id, spell, trans_ub) %>%
    summarize(mean_spell_dur = weighted.mean(inter_RT, 1:length(inter_RT)))
...
# data exampple
t = filter(map_type != "primitive")
# 
# t$map_0
# t$input_disp_0
# 
# ifelse(matches_input(t$map_0, t$input_disp_0), spell_output(t$map_0), t$input_disp_0)
...
```{=html}
<!--
(1) check hypotheses from your exposé 
