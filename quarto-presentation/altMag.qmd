...
responseVsFitted = function(model){
  resp = getResponse(model, level = 0)
...
The following analyses are based on the trials from the `single`and `double spell block`.
...
#| warning: false
gd = groupedData(
  log(rt) ~ cue_type + map_type + test_type + acc + trialNumTotal|id,
                 data = dualTrials)
...
-   A random slope may be useful for the factor `acc`.
...
```{r}
ri_model = lme(log(rt) ~ trialNumTotal + acc + cue_type + map_type + test_type,
...
linpredVsResiduals(ri_model) +
  scale_x_continuous(breaks = seq(-0.2, 0.5, 0.1), limits = c(-0.25, 0.55)) +
...
Overall more *under-* than *overestimation* but the mean structure seems to be correctly specified. We can now inspect the RI model summary.
...
-   The expected log RTs can be approximated adequately by a linear function of `map type`, `test type`, `accuracy` and `trial number`. A random intercept is warranted, random slopes do not appear necessary.

-   Reactions for `double spells` are significantly slower (regardless of reducibility). Reactions for `position tests` are also significantly slower (which may be an artifact: the response options are fixed for `count` trials, not for `position` trials).

-   Accurate responses are significantly faster. All responses become significantly *faster over time*, which may indicate *further learning* in session 2.
...
#| warning: false
gd = groupedData(
  log(inter_RT) ~ cue_type + map_type + test_type + trans_ub + acc + trialNumTotal|id, data = dualTrials)
lm_separate = nlme::lmList(
  log(inter_RT) ~ cue_type + map_type + test_type + trans_ub + acc + trialNumTotal|id, data = gd)
plot(intervals(lm_separate))

```

A random intercept and slopes for `trial number` seems adequate. The *ICC* is higher with `r round(icc, digits = 2)`.

```{r}
ri_model = lme(log(inter_RT) ~ cue_type + map_type + test_type + trans_ub + acc + trialNumTotal,
               random = ~ 1 | id, data = dualTrials)
...
This time an additional random slope is warranted by the marginal information criteria.

```{r}
rirs_model_ie = update(rirs_model, fixed = log(inter_RT) ~ cue_type + map_type + test_type + trans_ub + trans_delta + acc + trialNumTotal)
anova(rirs_model, rirs_model_ie)
```

However, the amount of possibly skip-able transformations (`trans_delta`) does not seem to improve the model fit sufficiently.

```{r}
#| warning: false
linpredVsResiduals(rirs_model) +
  scale_x_continuous(breaks = seq(0.2, 0.8, 0.1), limits = c(0.15, 0.85)) +
  scale_y_continuous(breaks = seq(-1.5, 2.0, 0.5), limits = c(-1.7, 2.2))
```
...
-   The odds of an accurate response *for a given subject* is `r round(exp(ri_gmodel@beta[1]), digits=2)` [^2].

-   They only seem to be sensitive above chance to `second-only` spells which decrease the odds (c.p.) by a factor of `r round(exp(ri_gmodel@beta[5]), digits=2)`.

-   While `generic` double spells decrease the odds (c.p.) by a factor of `r round(exp(ri_gmodel@beta[4]), digits=2)`, the effect does not reach statistical significance.

[^2]: At the start of the experiment for primitive trials with a text cue and count test.

```{=html}
<!-- 
(1) compositionality: RT for double spells based on average from used primitives vs other, maybe incorporate trial number as IE
(2) AHA moments (jump in RT for second-only)
(3) check hypotheses from your exposé 
-->
```
