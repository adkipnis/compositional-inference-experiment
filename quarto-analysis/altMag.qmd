...
### Models for transformation duration (interRT)
...
Finally, let's have a peek at the effects of the `trans_ub` and `trans_delta`regardless of other predictors:

```{r}
#| warning: False
#| label: inter-rt-pure-summary
#| code-fold: show
lme(log(inter_RT) ~ trans_ub + trans_delta,
    random = ~ 1 | id, data = dualTrialsClean) %>% summary()
```
...
-   The bigger the number of to be transformed objects (`trans_ub`) the *smaller* the processing time. This may be due to displays with multiple duplicate items (such that they can be transformed simultaneously)

-   If `trans_ub` and `trans_delta` are used as predictors in isolation, both increase the expected time to apply the functions. The latter is only non-zero for reducible double spells, which are more difficult for participants as we have previously seen.
...
matches_output = function(spell_char, output_char){
  output_spell = spell_output(spell_char)
  return(output_spell == output_char)
}

applySpell = function(spell_char, input_char){
  out = ifelse(matches_input(spell_char, input_char),
               spell_output(spell_char),
               input_char)
  return(out)
}
...
spell_input("A-B") == "A"
spell_output("A-B") == "B"
spell_input(dualTrialsClean$map_0[1:2])
matches_input("C-A", "B") == F
matches_input("B-A", "B") == T
applySpell("A-B", "A") == "B" 

# data example
t1 = dualTrialsClean %>% filter(map_type != "primitive") %>% slice(6) %>%
  mutate(mid_disp_0 = applySpell(map_0, input_disp_0),
         mid_disp_1 = applySpell(map_0, input_disp_1),
         mid_disp_2 = applySpell(map_0, input_disp_2),
         mid_disp_3 = applySpell(map_0, input_disp_3)) 
...
ri_model = lme(
  log(inter_RT) ~ - 1 + trialNumTotal + log(inter_RT_0) + log(inter_RT_1),
  random = ~ 1 | id,
  data = doubleSpells)
...
Now we try to infer the type of erroneous response for **position trials**:

1.  *Spell not applied* (input display at queried position contains input of cued spell AND given response is the same item)
2.  *Wrong spell applied* (the chosen item is the output of a spell which was not cued AND the input display had the spell input at the same position)
3.  *Unknown error* (else)

Similarly, for **count trials**:

1.  *Spell not applied* (count response matches the count of the queried item in the input display)
2.  *Wrong spell applied* (count response matches the count of the queried item in the output display after applying one of the other two spells)
3.  *Unknown error* (else)

```{r}
#| include: false
# infer potential spell that would lead to the given response:
singleSpellErrors = testTrials %>%
  filter(map_type == "primitive", acc == 0, !is.na(emp_resp)) %>%
  mutate(resp_item = case_when(
    emp_resp == 0 ~ resp_options_0,
    emp_resp == 1 ~ resp_options_1,
    emp_resp == 2 ~ resp_options_2,
    emp_resp == 3 ~ resp_options_3))

```

*Subroutine + unit tests to check if the cued spell was not applied:*

```{r}
#| label: spell-not-applied-subroutine

spellNotApplied = function(trial){
  if (trial["test_type"] == "position"){
    spell = trial["map_0"]
    queried_idx = trial["target"]
    input_item = eval(parse(text=paste0(
      "trial['input_disp_", queried_idx, "']")))
    chosen_item = trial["resp_item"]
    return(matches_input(spell, input_item) && input_item == chosen_item)
    
  } else if (trial["test_type"] == "count"){
    queried_item = trial["target"]
    chosen_count = trial["resp_item"]
    input_count = sum(sapply(trial[paste0("input_disp_", 0:3)],
                             function(x) x == queried_item))
    return(chosen_count == input_count)
  }
}

# one positive, one negative example for position
test = data.frame(
  test_type = rep("position", 2),
  map_0 = c("A-B", "C-D"),
  target = c(0, 1),
  input_disp_0 = c("A", "B"),
  input_disp_1 = c("B", "C"),
...
apply(test, 1, spellNotApplied)

# one positive, one negative example for count
test = data.frame(
  test_type = rep("count", 2),
  map_0 = c("A-B", "C-D"),
  target = c("A", "D"),
  input_disp_0 = c("A", "A"),
  input_disp_1 = c("B", "B"),
...
*Subroutine + unit tests to check if another spell may have been applied instead:*

```{r .output}
#| label: wrong-spell-applied-subroutine

spells = c("A-B", "B-A", "C-D")

wrongSpellApplied = function(trial){
  spell = trial["map_0"]
  other_spells = spells[-grep(spell, spells)]
  
  if (trial["test_type"] == "position"){
    chosen_item = trial["resp_item"]
    chosen_is_output = sapply(
      other_spells, function(s) matches_output(s, chosen_item))
    if (!any(chosen_is_output)) return(F)
    
    spell_candidate = names(chosen_is_output)[which(chosen_is_output)]
    queried_idx = trial["target"]
    input_item = eval(parse(text=paste0(
      "trial['input_disp_", queried_idx, "']")))
    return(matches_input(spell_candidate, input_item))
    
  } else if (trial["test_type"] == "count"){
    queried_item = trial["target"]
    chosen_count = trial["resp_item"]
    for (s in other_spells){
      alt_count = sum(sapply(trial[paste0("input_disp_", 0:3)],
                             function(x) applySpell(s, x) == queried_item))
      if (alt_count == chosen_count) return(T)
    }
    return(F)
  }
}
...
apply(test, 1, wrongSpellApplied)

# one positive, one negative example for count
# (this is the most difficult one to trace back)
test = data.frame(
  test_type = rep("count", 2),
  map_0 = c("A-B", "A-B"),
  applied_spell = c("B-A", "A-B"),
  target = c("A", "B"),
  input_disp_0 = c("A", "A"),
  input_disp_1 = c("B", "B"),
  input_disp_2 = c("C", "C"),
...
*Single spell subset with classified errors:*

```{r}
singleSpellErrors$noSpellMaybe = apply(singleSpellErrors, 1, spellNotApplied)
singleSpellErrors$wrongSpellMaybe = apply(singleSpellErrors, 1, wrongSpellApplied)
singleSpellErrors = singleSpellErrors %>%
  mutate(eitherError = noSpellMaybe & wrongSpellMaybe,
         unkownError = !noSpellMaybe & !wrongSpellMaybe) %>%
  mutate(error_type = ifelse(unkownError, "unkown",
                                                 ifelse(eitherError, "either",
                                                        ifelse(noSpellMaybe, "no spell", "wrong spell")
                                                        )
                                                 )
         )
singleSpellErrors %>% select(input_disp_0, input_disp_1, input_disp_2, input_disp_3,
                             spell, test_type, target, correct_resp, resp_item,
                             target, correct_resp, error_type)
```

Summary:

```{r}
singleSpellErrors %>% group_by(id, error_type) %>% summarize(n = n()) %>%
  ggplot(aes(x=error_type, y=n)) +
    ...
    scale_y_continuous(expand = c(0, 0), limits = c(0, 16),
                       breaks = seq(0, 16, 4)) +
    theme_minimal()+
    guides(fill = guide_legend(title = "Error Profiles in Single Spells"))+
    xlab("Error Type") +
    ylab("# occurrence") +
    theme(
      panel.border = element_rect(colour = "gray", fill=NA, size=1),
...
```{=html}
<!--
- error type analysis (e.g. confuse B-A)
-->
```
